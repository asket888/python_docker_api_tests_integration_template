...
from api_tests.errors.custom_errors import ResponseTimeoutError
from api_tests.utils.config_util import get_authorization


class TestSumLazyEndpoint(unittest.TestCase):
    _GET_SUM_LAZY_ENDPOINT_TEMPLATE = \
        "http://localhost:4000/sumLazy?a={first_value}&b={second_value}"

    # positive test
    def test_sum_lazy_with_positive_numbers(self):
        response = self._get_endpoint_response(first_value=3, second_value=11)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["result"], 14)

    def test_sum_lazy_with_negative_numbers(self):
        response = self._get_endpoint_response(first_value=3, second_value=-11)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["result"], -8)

    def test_sum_lazy_with_one_zero_number(self):
        response = self._get_endpoint_response(first_value=0, second_value=11)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["result"], 11)

    # negative test
    def test_sum_lazy_with_string_value_validation(self):
    ...
    def test_sum_lazy_with_decimal_value_validation(self):
        response = self._get_endpoint_response(first_value=3, second_value=11.0)
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.json()["error"], "Expected Numeric Param")

    def test_sum_lazy_with_long_value_validation(self):
        response = self._get_endpoint_response(first_value=0, second_value=18446744073709551615)
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.json()["error"], "Expected Numeric Param")

    def test_sum_lazy_with_missed_parameter_validation(self):
    ...
    def _get_endpoint_response(self, first_value, second_value):
        ...
        response = requests.get(
            url=self._GET_SUM_LAZY_ENDPOINT_TEMPLATE.format(first_value=first_value,
